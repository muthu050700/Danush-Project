...
<div class="row">
       ...
       con=dbcon.create();
       PreparedStatement ps=con.prepareStatement("SELECT * FROM `onpur`.`addveg` where statu='qa'");
...
<div class="card">
   ...
   <p class="card-text"> product Type: <%=rs.getString(5)%></p>
    ...
    <p class="card-text">Price per KG :<%=rs.getString(8)%></p>
       <p class="card-text">Status :<%=rs.getString(12)%></p>
    <a href="approved.jsp?id=<%=rs.getString(1) %>"><button type="button" class="btn btn-primary">Approve</button></a>  
 
 <a href="rejected.jsp?id=<%=rs.getString(1) %>"><button type="button" class="btn btn-primary">Reject</button></a>  
