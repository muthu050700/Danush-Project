...
<div class="row">
       ...
       con=dbcon.create();
       PreparedStatement ps=con.prepareStatement("SELECT * FROM `onpur`.`addveg` where local='"+location+"' and vid='"+id+"' and statu='approved' and Dtype='DELIVERY'");
...
<div class="card">
   ...
   <p class="card-text"> product Type: <%=rs.getString(6)%></p>
    ...
    <a href="Dcart.jsp?id=<%=rs.getString(1)%>&&email=<%=rs.getString(4)%>&&vid=<%=rs.getString(6)%>&&pname=<%=rs.getString(7)%>&&price=<%=rs.getString(9)%>&&pic=<%=rs.getString(10)%>&&cemail=<%=email%>&&Dtype=<%=rs.getString(13)%>&&add=<%=rs.getString(5)%>"><button style="width:100px; border-radius:20px;background-color:lightblue;margin-left:190px;">Add to cart</button></a>
      </div>
